<?php
/**
 * Pedicure Makeup Store Theme Customizer
 *
...
if ( ! function_exists( 'pedicure_makeup_store_customize_register' ) ) :
function pedicure_makeup_store_customize_register( $wp_customize ) {
    ...
    $wp_customize->register_control_type( 'Pedicure_Makeup_Store_Toggle_Control' );

    $wp_customize->get_setting( 'blogname' )->transport         = 'postMessage';
    $wp_customize->get_setting( 'blogdescription' )->transport  = 'postMessage';
    $wp_customize->get_setting( 'header_textcolor' )->transport = 'postMessage';
    $wp_customize->get_control( 'header_textcolor' )->label = __( 'Site Title Color', 'pedicure-makeup-store' );

    if ( isset( $wp_customize->selective_refresh ) ) {
        $wp_customize->selective_refresh->add_partial( 'blogname', array(
            'selector'        => '.site-title a',
            'render_callback' => 'pedicure_makeup_store_site_title_callback',
            'fallback_refresh'    => false,
        ) );
        $wp_customize->selective_refresh->add_partial( 'blogdescription', array(
            'selector'        => '.site-description',
            'render_callback' => 'pedicure_makeup_store_site_description_callback',
            'fallback_refresh'    => false, 
        ) );
    }
    ...
    // Display Site Title
        $wp_customize->add_setting( 
            'pedicure_makeup_store_display_site_title', 
            array(
                'default'           => true,
                'type'              => 'theme_mod',
        ...
        $wp_customize->add_control( 
            new Pedicure_Makeup_Store_Toggle_Control( 
                ...
                'pedicure_makeup_store_display_site_title', 
                array(
                    'label'    => esc_html__( 'Display Site Title', 'pedicure-makeup-store' ),
                    'section'  => 'title_tagline',
                    'settings' => 'pedicure_makeup_store_display_site_title',
                ) 
            )
        );
        ...
        $wp_customize->add_setting( 
            'pedicure_makeup_store_display_site_tagline', 
            array(
                'default'           => false,
                'type'              => 'theme_mod',
                'sanitize_callback' => 'pedicure_makeup_store_sanitize_checkbox',
        ...
        $wp_customize->add_control( 
            new Pedicure_Makeup_Store_Toggle_Control( 
                ...
                'pedicure_makeup_store_display_site_tagline', 
                array(
                    'label'    => esc_html__( 'Display Tagline', 'pedicure-makeup-store' ),
                    'section'  => 'title_tagline',
                    'settings' => 'pedicure_makeup_store_display_site_tagline',
                ) 
            )
        );

    // Add setting
    $wp_customize->add_setting( 'pedicure_makeup_store_logo_width', array(
        'default'           => 150,
    ...
    $wp_customize->add_control( 'pedicure_makeup_store_logo_width', array(
        'label'       => __( 'Logo Resizer (px)', 'pedicure-makeup-store' ),
        'section'     => 'title_tagline',
        'type'        => 'range',
        'input_attrs' => array(
            'min'  => 50,
...
add_action( 'customize_register', 'pedicure_makeup_store_customize_register' );
 ...
 */
if ( ! function_exists( 'pedicure_makeup_store_enqueue_customizer_stylesheets' ) ) :
function pedicure_makeup_store_enqueue_customizer_stylesheets() {
    wp_register_style( 'pedicure-makeup-store-customizer-css', get_template_directory_uri() . '/inc/customizer/assets/css/customizer.css', array(), '1.0.9', 'all' );
    wp_enqueue_style( 'pedicure-makeup-store-customizer-css' );
}
endif;
add_action( 'customize_controls_print_styles', 'pedicure_makeup_store_enqueue_customizer_stylesheets' );